...
export const Newsletter = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok || !data?.ok) {
        setError(data?.error ?? 'No pudimos registrarte ahora. Inténtalo en unos minutos.');
        return;
      }
      setIsSubmitted(true);
      setTimeout(() => {
      ...
      setError('No pudimos registrarte ahora. Inténtalo en unos minutos.');
    } finally {
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="relative">
                    ...
                    <motion.button
                      ...
                      className="absolute right-2 top-1/2 -translate-y-1/2 bg-[#0c3c1f] text-white px-6 py-2 rounded-lg hover:bg-[#0a3019] transition-colors font-semibold disabled:opacity-60 disabled:cursor-not-allowed"
                    >
                  ...
                  {error && (
                    <p className="text-red-600 text-xs pt-1" role="alert">
                      {error}
                    </p>
                  )}
                </form>
