...
// Obtener los productos más recientes (ordenados por created_at descendente)
export const getNewestProducts = async (first: number = 20): Promise<Product[]> => {
  try {
    const data: any = await shopifyClient.request(GET_NEWEST_PRODUCTS, {
      first,
      after: undefined,
    });
    ...
    console.error('Error fetching newest products from Shopify:', error);
    return [];
  }
};
